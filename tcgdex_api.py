...
class TCGdexAPI:
    ...
    def search_cards(self, card_name: str, set_name: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            print(f"Erreur TCGdex search_cards: {e}")
            return []
    ...
    def get_card(self, card_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if e.response.status_code == 404:
                return None
            print(f"Erreur TCGdex get_card: {e}")
            return None
        except Exception as e:
            print(f"Erreur TCGdex get_card: {e}")
            return None
    ...
    def search_card_with_prices(
        ...
        try:
            ...
            print(f"Erreur TCGdex search_card_with_prices pour '{card_name}': {e}")
            return None, None, None


def test_tcgdex_api():
    """Fonction de test pour vérifier l'API TCGdex"""
    print("🧪 Test de l'API TCGdex...")
    ...
    print(f"✅ Trouvé {len(cards)} cartes Pikachu")
    
    if cards:
        card = cards[0]
        print(f"   Premier résultat: {card.get('name')} - {card.get('id')}")
        ...
        if card_full:
            ...
            if price_avg:
                print(f"✅ Prix min: {price_avg}")
                print(f"✅ Prix max: {price_max}")
                ...
                if pricing.get('cardmarket'):
                    ...
                    print(f"   trend: {cm.get('trend')}€")
                    print(f"   avg: {cm.get('avg')}€")
                    print(f"   low: {cm.get('low')}€")
                ...
                print("⚠️ Pas de prix disponible")
        else:
    ...
    if price_avg:
        print(f"✅ Charizard Base Set trouvé!")
        print(f"   Prix min: {price_avg}")
        print(f"   Prix max: {price_max}")
        print(f"   Set: {details.get('set', {}).get('name')}")
    else:
        print("⚠️ Charizard Base Set non trouvé (normal, carte rare)")
